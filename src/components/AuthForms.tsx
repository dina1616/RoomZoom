...
const LoginForm: React.FC = () => {
  ...
  const [error, setError] = useState<string | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const { login, checkAuthStatus } = useAuth(); // Use login from context
  const router = useRouter();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update auth context state (optional, as checkAuthStatus will run on redirect/refresh)
      // login(data.user); 

      // Manually trigger auth status check after successful login 
      // before potential redirect by middleware
      await checkAuthStatus();
      
      // Redirect logic is handled by middleware, but can add fallback
      // router.push('/dashboard'); // Or check for redirect param from middleware
      // For now, let middleware handle redirect based on cookie being set.
      // A page refresh might be needed implicitly if middleware runs before context updates.
      // Consider using router.refresh() if needed.
